...
public class Quadrilateral implements Positionable, TwoDShape {
    ...
    protected double[] getSideLengths() {
        ...
        for (int i = 0; i < vertices.length - 1; i++) {
            if (i % 2 == 0) {
                double length =
                        Math.abs(vertices[i].coordinates()[0] - vertices[i + 1].coordinates()[0]);
                lengths[i] = length;
            } else {
                double length2 =
                        Math.abs(vertices[i].coordinates()[1] - vertices[i + 1].coordinates()[1]);
                lengths[i] = length2;
            }
        } // Stops at i = 2 and checks i = 3
        // Check at i = 3
        lengths[3] =
                Math.abs(vertices[3].coordinates()[1] - vertices[0].coordinates()[1]);
        return lengths;
        // CHECK
    }
